...
if [ ! -f "src/_setup_complete.flag" ]; then
    ...
    (cd database && chmod +x db_start_mac.sh && ./db_start_mac.sh) || { echo "[Warning] Could not start database."; }
    echo ""
...
(cd database && ./db_start_mac.sh) || { echo "[Warning] Database may not have started correctly."; }
echo ""
